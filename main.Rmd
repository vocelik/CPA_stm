...
knitr::opts_chunk$set(echo = T, message = F, error = F, dpi = 300, fig.width = 8,
...
many_models <- tibble(K = k) %>%
  mutate(topic_model = future_map(K, ~stm(documents = out$documents, vocab = out$vocab, 
                                          data = out$meta, K = . ,prevalence = ~ field + s(months_since_first_pub),
                                          verbose = FALSE)))
