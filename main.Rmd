...
Check which words were removed
```{r}
df_clean_final %>%
  unnest_tokens(word, abstract) %>%
  anti_join(stop_words) %>%
...
```{r eval = F}
# extracting topic model with 100 topics and three two variables for topic prevalence.
...
```{r Covariates}
number_of_topics <- tidy(df_stm) %>% pull(topic) %>% max()
...
```{r}
a <- labelTopics(df_stm, topics = c(1:60))
...
topic_thoughts <- findThoughts(df_stm, topic = c(1:60), texts = short_docs$abstract, n = 8)
```
