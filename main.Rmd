...
df_clean <- df %>%
  ...
  mutate(ID = row_number(),
         abstract = str_remove(abstract, "[(][C-c][)].*$"), # some abstracts have '(C) Elsevier' at the ending.
...
grid.arrange(cite_boxplot, cite_boxplot_log,
             ncol = 2,
             top = "Boxplot comparison of citations on regular and log scale.",
             left = "times cited")

grid.arrange(cite_hist, cite_hist_log,
             ncol = 2,
             top = "Histogram comparison of citations on regular and log scale.",
             left = "count",
...
df_clean %>%
  ...
  head(10) %>%
  kable(caption = "10 most common journals not matched with an academic field.") %>%
